...
export class Messaging extends Construct {
  ...
  constructor(scope: Construct, id: string, _props: MessagingProps) {
    super(scope, id);
    ...
    const { appInstanceArn, appInstanceAdminArn, channelFlowArn } =
      this.createAppInstanceCustomResource(role, appInstanceUserId, presence);
        ...
        CreateChannelFlowPolicy: new iam.PolicyDocument({
          statements: [
            new iam.PolicyStatement({
              ...
              resources: [
                `arn:aws:chime:${cdk.Stack.of(this).region}:${
  ...
  private createAppInstanceCustomResource = (
    ...
    const customResource = new cdk.CustomResource(
      this,
      'CreateAppInstanceCustomResource',
      {
    ...
    return {
      ...
      appInstanceAdminArn: customResource
        .getAtt('AppInstanceAdminArn')
        .toString(),
   ...
   */
  private createDeleteAppInstanceCustomResource = (
    appInstanceArn: string,
  ): void => {
    const role = this.createDeleteAppInstanceRole(appInstanceArn);
    const deleteAppInstanceLambdaFunctionHandler =
      new lambdaNodeJs.NodejsFunction(this, 'DeleteAppInstanceFunction', {
